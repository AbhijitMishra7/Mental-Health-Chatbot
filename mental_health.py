        ...
        if embedding_vector is not None:        
            ...
            for wrd in list:
                ...
                if embedding_vector is not None:        
                    ...
                    embedding_matrix[hits] = embedding_matrix[hits-1]
                    hits+=1
                    misses+=1
                    
        else:
            embedding_matrix[hits] = np.zeros(100)
            embedding_matrix[hits+1] = np.zeros(100)
            embedding_matrix[hits+2] = np.zeros(100)
            embedding_matrix[hits+3] = np.zeros(100)
            embedding_matrix[hits+4] = np.zeros(100)
            hits+=5
            misses+=5
        print(hits,misses)
...
for i in range(911):
    vec=np.nonzero(X_train[i,:])
    for j in vec[1]:
        pred_dict[i]=np.add(pred_dict[i], embedding_matrix[j*5])
        np.divide(pred_dict[i],3)
...
def chat():
    ...
    while True:
        ...
        str=" "
        str=str.join(inpt)
        data=cv.transform([str])
        ct=np.nonzero(data)
        pre=np.zeros((1,100))
        for k in ct[1]:
            pre[0]=np.add(pre[0], embedding_matrix[k*5])
            pre[0]=np.divide(pre[0],3)
        max=0
        index=-1
        pre.reshape(-1,1)
        for i in range(911):
